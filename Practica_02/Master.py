...
class GUIClock:		#La GUI del reloj estara definida en esta clase
	...
	def setTimeGUI_Horas(self,win,horas): #Funcion que establece los valore del reloj
		self.clk.h = int(horas)
		self.clk.status=True
		self.lbl = Label(win, text="%02d:%02d:%02d" % (self.clk.h , self.clk.m , self.clk.s))
	def setTimeGUI_Mins(self,win, minutos): #Funcion que establece los valore del reloj
		self.clk.m = int(minutos)
		self.clk.status=True
		self.lbl = Label(win, text="%02d:%02d:%02d" % (self.clk.h , self.clk.m , self.clk.s))
	def setTimeGUI_Segs(self,win, segundos): #Funcion que establece los valores del reloj
		self.clk.s = int(segundos) - 1
		self.clk.status=True
		self.lbl = Label(win, text="%02d:%02d:%02d" % (self.clk.h , self.clk.m , self.clk.s))
	def popup_clock_config(self,win, ElemAModificar):#Funcion para la modificacion de los valores del reloj con GUI
		...
		if ElemAModificar == 0:
			...
			b1 = Button(ven, text= "Cambiar horas", command= lambda: GUIClock.setTimeGUI_Horas(self,win,entradaHoras.get() ) or ven.destroy()  )
			...
			entradaminutos.grid(row=1, column=1)
			b1 = Button(ven, text= "Cambiar Minutos", command= lambda: GUIClock.setTimeGUI_Mins(self,win,entradaminutos.get() ) or ven.destroy()  )
			b1.grid(row=2, column=0)
			...
			entradaSegs.grid(row=1, column=1)
			b1 = Button(ven, text= "Cambiar Segundos", command= lambda: GUIClock.setTimeGUI_Segs(self,win,entradaSegs.get() ) or ven.destroy()  )
			b1.grid(row=2, column=0)
	...
	def setSegGUI(self, seg):
		...
		self.clk.status = True
class GUIClockRandMaster(GUIClock): #Clase hija de GUIClock, la unica diferencia es que este inicializa un reloj aleatorio
	def __init__(self, win, _x , _y):
		self.clk = randclock()
		#win.title("Window")
		self.lbl = Label(win, text="%02d:%02d:%02d" % (self.clk.h , self.clk.m , self.clk.s))
		self.lbl.grid(row = _x , column = _y, columnspan=2)
		self.btn = Button(win, text ="Modificar horas", command = lambda: GUIClock.popup_clock_config(self,win, 0)  )
		self.btn.grid(row = _x+1, column = _y)
		self.btn = Button(win, text ="Modificar minutos", command = lambda: GUIClock.popup_clock_config(self,win, 1)  )
		self.btn.grid(row = _x+2, column = _y)
		self.btn = Button(win, text ="Modificar segundos", command = lambda: GUIClock.popup_clock_config(self,win, 2)  )
...
clk1 = GUIClockRandMaster(win,0,0)	#iniciamos el reloj maestro en la posicion 0, 0
